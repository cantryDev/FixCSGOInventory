...
function CreateItemContextMenuButton(elItemHolder, strCompositeId, owner) {
    ...
    var elActionMenuButton = new Element('a', {
        'id': strCompositeId + '_actionmenu_button',
        'class': 'slot_actionmenu_button'
    });
            ...
            inventory.LoadUntilConditionMet(function () {
                if ($Page.children().children('.item.pendingItem').length == 0) {
                    window.setTimeout(function () {
                        _this.m_bImagesLoaded = false;
                        _this.LoadPageImages()
                    }, 10);
                    return true;
                } else {
                    return false;
...
CInventory.prototype.ShowInventoryLoadError = function () {
    if (!this.m_$ErrorDisplay) {
        ...
        var _this = this;
        this.m_$ErrorDisplay.find('.retry_load_btn').click(function () {
            _this.RetryLoad();
        });
...
CInventory.prototype.ShowInventoryReloadMessage = function () {
    ...
    if (!this.m_$ReloadDisplay) {
        ...
        var _this = this;
        this.m_$ReloadDisplay.find('.reload_btn').click(function () {
...
CInventory.prototype.LoadCompleteInventory = function () {
    ...
    if (!this.m_promiseLoadCompleteInventory) {
        var _this = this;
        this.m_promiseLoadCompleteInventory = this.LoadUntilConditionMet(function () {
            return _this.m_bFullyLoaded;
        }, 5000 /* a lot at a time */);
    }
...
CInventory.prototype.NextPage = function () {
    if (this.m_iCurrentPage < this.m_cPages - 1 && !this.m_$Inventory.hasClass('paging_transition')) {
        ...
        var _this = this;
        this.m_fnQueuedPageTransition = function () {
            _this.NextPage();
        };
    }
};
...
CInventory.prototype.LocateAsset = function (assetid) {
    ...
    if (asset) {
        ...
        this.LoadUntilConditionMet(function () {
            return _this.m_rgAssets[assetid] != null;
        })
            .done(function () {
                deferred.resolve(_this.m_rgAssets[assetid]);
            })
            .fail(function () {
                deferred.reject();
            });
...
CInventory.prototype.SelectItem = function (event, elItem, rgItem, bUserAction) {
    ...
    elOldInfo.blankTimeout = window.setTimeout(function () {
        $(sOldInfo + '_item_icon').src = 'https://steamcommunity-a.akamaihd.net/public/images/trans.gif';
    }, 200);
...
CAppwideInventory.prototype.PerformInitialLoad = function () {
    ...
    for (var contextid in this.m_rgChildInventories) {
        ...
        if (inventory.m_cItems > 0) {
            var _this = this;
            return inventory.PerformInitialLoad().done(function () {
                _this.m_bPerformedInitialLoad = true;
            });
        }
    }
...
var g_rgEconomyDisplay = {
    "440": {"howtoget": "You can get them from free in-game item drops, the in-game Mann Co. Store, or trade for them with friends."},
    "620": {"howtoget": "You can get them from the Portal 2 in-game store or trade for them with friends."},
    "753": {"1": {"howtoget": "You can get extra copies of games during special promotions, or by purchasing a game from the Steam Store and selecting \"Purchase as a Gift\" at checkout time."}},
    "99900": {"logo": "https:\/\/steamcommunity-a.akamaihd.net\/public\/images\/economy\/applogos\/99900.png"},
...
function MouseOverItem(event, owner, elItem, rgItem) {
    ...
    if (hover.hiding && hover.visible() && hover.target == elItem) {
        ...
        if (hover.visible())
            window.setTimeout(function () {
                if (elItem.wants_hover) BuildHover('hover', rgItem, owner);
            }, Math.min(250, ITEM_HOVER_DELAY - 50));
        else
            BuildHover('hover', rgItem, owner);

        elItem.timer = window.setTimeout(function () {
            elItem.timer = false;
            if (elItem.wants_hover) ShowHover(elItem, rgItem);
        ...
        if (description.fraudwarnings || (g_bIsInventoryPage && item.is_currency)) {
            ...
            if (description.fraudwarnings) {
                for (var i = 0; i < description.fraudwarnings.length; i++) {
                    ...
                    var warningImage = $J('<img>', {
                        class: 'fraud_warning_image',
                        src: 'https://steamcommunity-a.akamaihd.net/public/images/sharedfiles/icons/icon_warning.png'
                    });
...
function CreateMarketActionButton(color, href, text) {
    var elButton = new Element('a', {
        'class': 'item_market_action_button item_market_action_button_' + color,
        'href': href
    });
...
function PopulateMarketActions(elActions, item) {
    ...
    if ((typeof (g_bViewingOwnProfile) != 'undefined' && g_bViewingOwnProfile) || bIsTrading) {
        ...
        var elBuybackInfoContent = new Element('div', {
            'id': buybackid,
            'class': 'market_item_action_buyback_at_price'
        });
...
function ShowItemHoverAsPopup(elNewInfo, fnOnDismiss) {
    ...
    if (g_ActiveItemPopupModal.m_fnBackgroundClick) {
        $Modal.add($Scroll).click(function (e) {
            if (e.target == this && g_ActiveItemPopupModal) g_ActiveItemPopupModal.m_fnBackgroundClick();
        });
...
SellItemDialog = {
    ...
    Show: function (item) {
        ...
        var _this = this;
        $J(this.m_elDialogContent).find('.newmodal_close').click(function () {
            _this.m_modal.m_fnBackgroundClick();
        });
    ...
    OnAccept: function (event) {
        ...
        new Effect.Morph('market_sell_quantity_input', {
            style: 'border-color: #262626; background-color: #262626',
            duration: 0.25
        });
        new Effect.Morph('market_sell_currency_input', {
            style: 'border-color: #262626; background-color: #262626',
            duration: 0.25
        });
        new Effect.Morph('market_sell_buyercurrency_input', {
            style: 'border-color: #262626; background-color: #262626',
            duration: 0.25
        });
    ...
    OnConfirmationBack: function (event) {
        ...
        new Effect.Morph('market_sell_quantity_input', {
            style: 'border-color: #707070; background-color: #1A1A1A',
            duration: 0.25
        });
        new Effect.Morph('market_sell_currency_input', {
            style: 'border-color: #707070; background-color: #1A1A1A',
